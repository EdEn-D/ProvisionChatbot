...
class LoadConfig:
    ...
    def __init__(self) -> None:
        ...
        self.persist_directory = str(here(app_config["directories"]["persist_directory"]))  # needs to be strin for summation in chromadb backend: self._settings.require("persist_directory") + "/chroma.sqlite3"
        # self.custom_persist_directory = str(here(app_config["directories"]["custom_persist_directory"]))
        self.embedding_model = OpenAIEmbeddings()

        # Retrieval configs
        self.data_directory_texts = app_config["directories"]["data_directory_texts"]
        self.data_directory_pdfs = app_config["directories"]["data_directory_pdfs"]
